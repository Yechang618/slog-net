...
#             self.validationInterval = data.nTrain//batchSize
...
#         YValid = (data.samples)['valid']['signals']
#         YValid = torch.reshape(torch.transpose(YValid, 0, 2),(self.nNodes,self.nValid))
#         yValid = torch.reshape(YValid, (self.nNodes*self.nValid,))
...
#         YTrain = (data.samples)['train']['signals']
#         YTrain = torch.reshape(torch.transpose(YTrain, 0, 2),(self.nNodes,self.nTrain))
#         yTrain = torch.reshape(YTrain, (self.nNodes*self.nTrain,))
...
#         sampleTime_test = (data.samples)['test']['sampleTimes']
...
#                 XTrainBatch = SLOGdata.to_numpy(XTrainBatch)
