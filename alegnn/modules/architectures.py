...
class SelectionGNN(nn.Module):
    ...
    def __init__(self,
        ...
        if load_coarsened_GSO == True:
            self.load_coarsened_GSO = load_coarsened_GSO
            self.coarsened_outputs = coarsened_outputs
            self.coarsened_GSO = coarsened_outputs['coarsened_GSO']
            self.coarsened_order = coarsened_outputs['coarsened_order']
        else:
            self.coarsened_GSO = None
            self.coarsened_order = None
            self.coarsened_outputs = {}
            self.load_coarsened_GSO = load_coarsened_GSO
...
#             self.permFunction = eval('Utils.graphTools.perm' + order)
        ...
        if self.coarsening and self.E == 1:
            ...
            if self.load_coarsened_GSO == False:
                GSO, self.order = alegnn.utils.graphTools.coarsen(GSO, levels=self.L,
                                                              self_connections=False)
                self.coarsened_GSO = GSO
                self.coarsened_order = self.order 
                self.coarsened_outputs['coarsened_GSO'] = GSO
                self.coarsened_outputs['coarsened_order'] = self.order 
            else:
                GSO = self.coarsened_GSO
                self.order = self.coarsened_order
